...
const BASE_URL = "https://www.domicile.co.il";

export async function scrapeDomicileProduct(
  url: string
  ...
  $("select.variation-selector option, table.variations select option").each(
    (_, el) => {
  ...
  const category =
    $(".posted_in a").first().text().trim() || "ידיות";
...
export async function scrapeDomicileCategory(
  url: string
